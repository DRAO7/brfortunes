...
export default function Post({
  ...
  const Wrapper = postId
    ? ({ children }: { children: React.ReactNode }) => (
        <Link href={`/posts/${postId}`}>{children}</Link>
      )
    : ({ children }: { children: React.ReactNode }) => <>{children}</>;

  return (
    <Wrapper>
      <div className="w-[859px] p-2.5 rounded-[10px] outline outline-1 outline-offset-[-1px] outline-black/5 inline-flex flex-col justify-start items-start gap-[5px] bg-white cursor-pointer">
        {/* HEADER */}
